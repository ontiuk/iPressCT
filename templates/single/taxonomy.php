...
if ( in_array( get_post_type(), $ip_footer_taxonomy_types, true ) ) {
	...
	// Get category list
	$category_list = ( has_category() ) ? sprintf( '<span class="post-categories">%1$s %2$s</span>', esc_html__( 'Posted in', 'ipress-child' ), get_the_category_list( ', ' ) ) : '';

	// Get the tag list
	$tag_list = ( has_tag() ) ? sprintf( '<span class="post-tags">%1$s %2$s</span>', esc_html__( 'Tagged in', 'ipress-child' ), get_the_tag_list( '', ', ' ) ) : '';
