...
if ( ! class_exists( 'IPR_Hooks' ) ) :
	...
	final class IPR_Hooks extends IPR_Registry {
		...
		public function the_archive_title( $title ) {
			...
			// Category
			if ( is_category() ) {
				return sprintf( __( 'Category: %s', 'ipress-child' ), single_cat_title( '', false ) );
			}

			// Author
			if ( is_author() ) {
				return sprintf(	__( 'Author: <span class="post-author">%s</span>', 'ipress-child' ), get_the_author() );
			}

			// Tag
			if ( is_tag() ) {
				return sprintf(	__( 'Tag: %s', 'ipress-child' ), single_tag_title( '', false ) );
			}

			// Taxonomy
			if ( is_tax() ) {
				$the_tax = get_taxonomy( get_queried_object()->taxonomy );
				return sprintf(	__( 'Taxonomy: %1$s: %2$s', 'ipress-child' ), esc_attr( $the_tax->labels->singular_name ), esc_html( single_term_title( '', false ) ) );
			}
