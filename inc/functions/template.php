...
if ( ! function_exists( 'ipress_post_date' ) ) :
	...
	function ipress_post_date( $datetime = true ) {
		...
		// Set the post prefix
		$ip_post_date_prefix = apply_filters( 'ipress_post_date_prefix', __( 'Posted on ', 'ipress-child' ) );
...
if ( ! function_exists( 'ipress_get_post_time' ) ) :
	...
	function ipress_get_post_time() {
		...
		return sprintf(
			...
			__( 'Posted on <a href="%1$s" title="%2$s" rel="bookmark">%3$s</a>', 'ipress-child' ),
...
if ( ! function_exists( 'ipress_post_author' ) ) :
	...
	function ipress_post_author() {
		...
		$byline = sprintf(
			...
			esc_attr( sprintf( __( 'View all posts by %s', 'ipress-child' ), get_the_author() ) ),
...
if ( ! function_exists( 'ipress_post_author_avatar' ) ) :
	...
	function ipress_post_author_avatar() {
		echo sprintf(
			...
			esc_html__( 'Written by', 'ipress-child' ),
...
if ( ! function_exists( 'ipress_post_categories' ) ) :
	...
	function ipress_post_categories() {
		...
		$ip_cat_term_separator = apply_filters( 'ipress_cat_term_separator', _x( ', ', 'Used between category list items.', 'ipress-child' ), 'categories' );

		// Set the category list prefix
		$ip_category_list_prefix = apply_filters( 'ipress_cat_list_prefix', esc_html__( 'Posted in', 'ipress-child' ) );
		...
		if ( $category_list ) {
			echo sprintf(
				...
				esc_html_x( 'Categories', 'Used before category list.', 'ipress-child' ),
				$category_list,
...
if ( ! function_exists( 'ipress_post_tags' ) ) :
	...
	function ipress_post_tags() {
		...
		$ip_tag_term_separator = apply_filters( 'ipress_tag_term_separator', _x( ', ', 'Used between tag names.', 'ipress-child' ), 'categories' );

		// Set the tag name prefix
		$ip_tag_list_prefix = apply_filters( 'ipress_tag_name_prefix', esc_html__( 'Tagged in', 'ipress-child' ) );
		...
		if ( $tag_list ) {
			echo sprintf(
				...
				esc_html_x( 'Tags', 'Used before tag names.', 'ipress-child' ),
				$tag_list,
				$ip_tag_list_prefix
			); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped in function.
		}
...
if ( ! function_exists( 'ipress_post_comments_link' ) ) :
	...
	function ipress_post_comments_link() {
		...
		$ip_comments_link_prefix = apply_filters( 'ipress_comments_link_prefix', esc_html__( 'Comments', 'ipress-child' ) );
		...
		if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			echo sprintf(
				...
				comments_popup_link( __( 'Leave a comment', 'ipress-child' ), __( '1 Comment', 'ipress-child' ), __( '% Comments', 'ipress-child' ) )
			); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped in function.
		}
	}
endif;
