...
//   'theme' => [
//     'app' => [ IPRESS_CHILD_ASSETS_URL . '/js/app.js', [ 'jquery' ], NULL ]
//   ],
//
...
//		],
		...
		// Theme scripts
		'theme' => [
			'navigation' => [ IPRESS_CHILD_ASSETS_URL . '/js/navigation' . $ip_suffix . '.js', [ 'app' ], $ipress_version, true ],
			'app' => [ IPRESS_CHILD_ASSETS_URL . '/js/app' . $ip_suffix . '.js', [ 'jquery' ], $ipress_version, true ],
		],
...
// return [
//   'cpt' => [
//     'singular' => _x( 'CPT', 'Post Type Singular Name', 'ipress-child' ),
...
// return [
//   'cpt_tax' => [
//     'singular' => _x( 'Taxonomy Name', 'Taxonomy Singular Name', 'ipress-child' ),
...
if ( is_admin() ) {
	require_once IPRESS_CHILD_INCLUDES_DIR . '/lib/acf-config.php';
}
...
if ( ipress_wc_active() ) {
	...
	add_filter( 'ipress_styles', function( $styles ) use( $ip_suffix ) {
		$styles['store'] = [
			'ipress-woocommerce' => [ 'all', IPRESS_CHILD_ASSETS_URL . '/css/woocommerce/woocommerce' . $ip_suffix . '.css', [ 'ipress' ], null ],
		];
		return $styles;
	}, 12 );
	...
	add_filter( 'ipress_login_page', function() {
		return ( current_user_can( 'manage_options' ) ) ? '' : __( 'my-account', 'ipress-child' );
	} );
