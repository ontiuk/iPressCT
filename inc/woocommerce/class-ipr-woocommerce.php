...
//			add_action( 'woocommerce_after_customer_login_form', [ $this, 'woocommerce_account_container_end' ], 10 );
		...
		public function woocommerce_custom_styles() {
			...
			if ( true === $ip_wc_custom_styles ) {
				...
				wp_register_style( 'ipress-woocommerce', IPRESS_CHILD_ASSETS_URL . '/css/woocommerce/woocommerce' . $ip_suffix . '.css', $ip_wc_custom_styles_dependency , $ipress_version );
				...
				if ( is_woocommerce() || is_cart() || is_checkout() ) {
					...
					// Add custom WooCommerce style: load after general WooCommerce style if there
					wp_register_style( 'ipress-woocommerce', IPRESS_CHILD_ASSETS_URL . '/css/woocommerce/woocommerce' . $suffix . '.css', $ip_wc_custom_styles_dependency, $ipress_version );
					wp_enqueue_style( 'ipress-woocommerce' );
					wp_style_add_data( 'ipress-woocommerce', 'rtl', 'replace' );
