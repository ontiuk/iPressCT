<?php

/**
 * iPress - WordPress Theme Framework
 * ==========================================================
 *
 * Theme functions & functionality.
 *
 * @package iPress\Functions
 * @link    http://ipress.uk
 * @license GPL-2.0+
 */

// Load Theme settings functions
include_once IPRESS_CHILD_INCLUDES_DIR . '/functions/settings.php';

// Load template functions
include_once IPRESS_CHILD_INCLUDES_DIR . '/functions/template.php';

// Load Hero section functions
include_once IPRESS_CHILD_INCLUDES_DIR . '/functions/hero.php';

// Load WooCommerce functions
include_once IPRESS_CHILD_INCLUDES_DIR . '/functions/product.php';

// Load Advanced Custom Fields functions
include_once IPRESS_CHILD_INCLUDES_DIR . '/functions/acf.php';

// Load Kirki functions
include_once IPRESS_CHILD_INCLUDES_DIR . '/functions/kirki.php';
