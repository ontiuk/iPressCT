		...
		<header class="page-header">
			...
			if ( 0 === $wp_query->found_posts ) {
				$search_result = sprintf(
					...
					_x( 'Search: No Results for <span class="search-query">%1$s</span>', 'Search results count', 'ipress-child' ),
					esc_html( get_search_query() )
				);
			} else {
				$search_result = sprintf(
					/* translators: 1. search results count, 2. search query. */
					_nx( 'Search: %1$s Result for <span class="search-query">%2$s</span>', 'Search: %1$s Results for <span class="search-query">%2$s</span>', $wp_query->found_posts, 'Search results count', 'ipress-child' ),
